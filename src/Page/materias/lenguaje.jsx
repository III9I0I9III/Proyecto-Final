...
import Ejercicio1 from "../ejercicios/Lenguaje/EjercicioLenguaje1";
import Ejercicio2 from "../ejercicios/Lenguaje/EjercicioLenguaje2";
import Ejercicio3 from "../ejercicios/Lenguaje/EjercicioLenguaje3";
import Ejercicio4 from "../ejercicios/Lenguaje/EjercicioLenguaje4";
...
function Lenguaje() {
  ...
  const salir = () => {
    ...
    const mensaje = `📚 Tu nota en Lenguaje es: ${nota}/10\n${mensajeFinal()}`;
    alert(mensaje);
    window.location.href = "/";
  };

  const calcularNotaFinal = () => {
    const respuestasUnicas = new Set(respuestasCorrectas);
    const nota = (respuestasUnicas.size / TOTAL_EJERCICIOS) * 10;
    return nota.toFixed(1);
  };

  const mensajeFinal = () => {
    const nota = parseFloat(calcularNotaFinal());
    if (nota === 10) return "🏆 ¡Perfecto! Dominas completamente los conceptos lingüísticos.";
    if (nota >= 7) return "🎯 ¡Muy bien! Solo algunos pequeños errores gramaticales.";
    if (nota >= 5) return "📖 Buen trabajo, pero hay aspectos que necesitas reforzar.";
    return "⚠️ Necesitas estudiar más los conceptos de lenguaje.";
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-100 p-6 flex flex-col justify-center items-center">
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="mt-8 text-center text-gray-500 text-sm">
          <p>© {new Date().getFullYear()} Academia de Lenguaje - Todos los derechos reservados</p>
        </div>
      </div>
    </div>
  );
